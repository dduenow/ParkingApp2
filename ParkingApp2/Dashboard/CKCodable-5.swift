...
class Cloud {
    internal static let key = CodingUserInfoKey(rawValue: "Zone_CloudKitWrapper_WeakReferenceObject")!
    ...
    fileprivate var zoneSaved: Bool = false
    ...
    // MARK: - CKRecord Cache
    fileprivate static var cachedSavedRecords: [CKRecordID: CKRecord] = [:]
    ...
    static func defaultPublicDatabase() -> Cloud {
        return Cloud(container: CKContainer.default(), database: CKContainer.default().publicCloudDatabase)
    }
    
    static func defaultPrivateDatabase(inZoneNamed zoneName: String) -> Cloud {
        return Cloud(container: CKContainer.default(), database: CKContainer.default().privateCloudDatabase, zone: CKRecordZone(zoneName: zoneName))
    }
    
    static func publicDatabase(withContainerIdentifier containerIdentifier: String) -> Cloud {
        let container = CKContainer(identifier: containerIdentifier)
        return Cloud(container: container, database: container.publicCloudDatabase)
    }
    
    static func privateDatabase(withContainerIdentifier containerIdentifier: String, inZoneNamed zoneName: String) -> Cloud {
        let container = CKContainer(identifier: containerIdentifier)
        return Cloud(container: container, database: container.privateCloudDatabase, zone: CKRecordZone(zoneName: zoneName))
    }
    ...
    func save<T: CloudKitEncodable>(_ objects: T, completionHandler: @escaping (Error?) -> Void) {
        saveZone { (cloud, error) in
            if let cloud = cloud {
                do {
                    let encoder = CloudKitEncoder()
                    encoder.zoneID = cloud.zone?.zoneID
                    ...
                    operation.modifyRecordsCompletionBlock = { (records, _, error) in
                        DispatchQueue.main.async {
                            if let error = error {
                                ...
                                Cloud.cachedSavedRecords = records.reduce(into: Cloud.cachedSavedRecords, { $0[$1.recordID] = $1 })
    ...
    func retrieveObjects<T: CloudKitDecodable>(with predicate: NSPredicate = NSPredicate(value: true), completionHandler: @escaping ([T], Error?) -> Void) {
        saveZone { (cloud, error) in
            if let cloud = cloud {
                cloud.retrieveRecords(for: String(describing: T.self), with: predicate) { (records, error) in
                    DispatchQueue.main.async {
                        if let error = error {
                            completionHandler([], error)
                        } else {
                            let decoder = CloudKitDecoder()
                            decoder.userInfo = [ Cloud.key : self ]
                            
                            do {
                                completionHandler(try decoder.decode([T].self, from: records), nil)
                            } catch {
                                completionHandler([], error)
                            }
                        }
                    ...
                    operation.modifyRecordsCompletionBlock = { (_, recordIDs, error) in
                        DispatchQueue.main.async {
                            if let error = error {
                                ...
                                Cloud.cachedSavedRecords = recordIDs.reduce(into: Cloud.cachedSavedRecords, { $0[$1] = nil })
                                completionHandler(nil)
                            } else {
                                completionHandler(nil)
                            }
                ...
                if let record = record {
                    self.container.discoverUserIdentity(withUserRecordID: record, completionHandler: { (userID, error) in
                        if let userID = userID {
                            completionHandler(Cloud.User(userId: userID), nil)
                        } else {
    ...
    fileprivate func saveZone(completionHandler: @escaping (Cloud?, Error?) -> Void) {
        guard let recordZone = self.zone, !zoneSaved else {
            completionHandler(self, nil)
            return
        }
        
        self.database.save(recordZone) { (recordZone, error) in
            self.zoneSaved = true
            if let error = error {
                completionHandler(nil, error)
            } else {
    ...
    fileprivate func retrieveRecords(for recordType: String, with predicate: NSPredicate = NSPredicate(value: true), completionHandler: @escaping ([CKRecord], Error?) -> Void) {
        ...
        var cachedRecords = Cloud.cachedSavedRecords.filter({ $1.recordType == recordType && predicate.evaluate(with: $1) })
        ...
        database.perform(query, inZoneWith: zone?.zoneID) { (cloudRecords, error) in
            if let cloudRecords = cloudRecords {
                ...
                self.retrieveChildrenRecords(of: cloudRecords, completionHandler: { (records, errors) in
                    records.forEach({ cachedRecords[$0.recordID] = $0 })
    ...
    fileprivate func retrieveChildrenRecords(of records: [CKRecord], completionHandler: @escaping ([CKRecord], [Error]) -> Void) {
        let parentDictionary = Dictionary(grouping: records, by: { $0.recordType })
        let promise = Promise<[CKRecord]>()
        
        if !parentDictionary.isEmpty {
            for parentType in parentDictionary.keys {
                let parentRecords = parentDictionary[parentType] ?? []
                let dependencies = Set(parentRecords.reduce(into: [], { $0 += ($1.object(forKey: "dependencyRecordTypesKey") as? Array<String> ?? []) }))
                
                if !dependencies.isEmpty {
                    for dependency in dependencies {
                        let dependencyComponents = dependency.components(separatedBy: "_")
                        
                        if let childType = dependencyComponents.first {
                            let key = Array(dependencyComponents.dropFirst()).joined(separator: "_")
                            let predicateElements = parentRecords.map({ return NSPredicate(format: "\(parentType)_\(key) == %@", $0.recordID) })
                            let predicate = NSCompoundPredicate(andPredicateWithSubpredicates: predicateElements)
                            
                            promise.then({ (records, next, finish, _) in
                                self.retrieveRecords(for: childType, with:predicate, completionHandler: {
                                    if let error = $1 {
                                        print(error)
                                    } else {
                                        next(records + $0)
...
fileprivate class Promise<T> {
    typealias NextFunctionType = (T, @escaping ((T) -> Void), @escaping ((T) -> Void), Progress) throws -> ()
    typealias FinishFunctionType = (T, @escaping (() -> Void), Progress) -> ()
    
    fileprivate struct Next {
        var progressCount: Int64
        var queue: DispatchQueue
        var function: NextFunctionType
    }
    
    fileprivate struct Finish {
        var progressCount: Int64
        var queue: DispatchQueue
        var function: FinishFunctionType
    }
    
    fileprivate struct OnError {
        var queue: DispatchQueue
        var function: (Error) -> ()
    }
    
    fileprivate var next: [Next]
    fileprivate var previous: Next?
    fileprivate(set) var progress: Progress
    
    fileprivate var finishFunction: Finish?
    fileprivate var catchFunction: OnError?
    
    init(progressCount: Int = 1, queue: DispatchQueue = .main, _ function: NextFunctionType? = nil ) {
        if let function = function {
            let next = Next(progressCount: Int64(progressCount), queue: queue, function: function)
            self.next = [next]
            self.progress = Progress(totalUnitCount: next.progressCount)
        } else {
            self.next = []
            self.progress = Progress(totalUnitCount: Int64(progressCount))
        }
    }
    
    @discardableResult func then(progressCount: Int = 1, queue: DispatchQueue = .main, _ function: @escaping NextFunctionType ) -> Self {
        let next = Next(progressCount: Int64(progressCount), queue: queue, function: function)
        self.next.append(next)
        self.progress.totalUnitCount = self.next.reduce(0, { $0 + $1.progressCount })
        
        return self
    }
    
    @discardableResult func finish(progressCount: Int = 0, queue: DispatchQueue = .main, _ function: @escaping FinishFunctionType) -> Self {
        if let previousFinish = finishFunction {
            self.progress.totalUnitCount -= previousFinish.progressCount
        }
        
        let finish = Finish(progressCount: Int64(progressCount), queue: queue, function: function)
        
        self.finishFunction = finish
        self.progress.totalUnitCount += finish.progressCount
        
        return self
    }
    
    @discardableResult func `catch`(queue: DispatchQueue = .main, _ function: @escaping (Error) -> ()) -> Self {
        self.catchFunction = OnError(queue: queue, function: function)
        
        return self
    }
    
    func begin(initialValue value: T) {
        guard !next.isEmpty else {
            finish(value: value)
            return
        }
        
        progress.completedUnitCount += previous?.progressCount ?? 0
        
        let nextFunction = next.removeFirst()
        
        nextFunction.queue.async {
            do {
                try nextFunction.function(value, self.begin, self.finish, self.progress)
            } catch let error {
                if let catchFunction = self.catchFunction {
                    catchFunction.queue.async {
        ...
        previous = nextFunction
    }
    
    fileprivate func finish(value: T) {
        guard let finishFunction = self.finishFunction else { return }
        
        finishFunction.queue.async {
            finishFunction.function(value, self.completed, self.progress)
        }
    }
...
extension CloudKitInformation: Decodable {
    public init(from decoder: Decoder) throws {
        if let decoder = decoder as? _CKDecoder {
            ...
            self.cloud = decoder.userInfo[Cloud.key] as? Cloud
